...
class User(UserMixin, db.Model):
	...
	def create_administrator():
		u = User(email = 'mchonghui@gmail.com',
				username = 'mch',
				password = '501825',
				admin = True)
		db.session.add(u)
...
class Blog(db.Model):
	...
	def create_about_blog():
		if Blog.query.filter_by(id=998).first() is None:
			...
			b = Blog(id=998, name='关于本站', summary='asdfasf', content='asdfsf', author=u)
			db.session.add(b)
			db.session.commit()
	@staticmethod
		...
		for i in range(count):
			try:
				b = Blog(name = forgery_py.lorem_ipsum.title(1),
					...
					create_at = forgery_py.date.date(True), author = u)
				db.session.add(b)
				db.session.commit()
...
class Label(db.Model):
	...
	def generate_fake(count=3):
		...
		if not Label.query.count():
			L = ['linux', 'Python', '数据结构与算法', '读书', 'Misc', 'Mysql', 'Flask']
			for labname in L:
